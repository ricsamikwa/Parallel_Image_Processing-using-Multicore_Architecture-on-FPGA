...
CORE_DIR=../../../quartus/H4_
CORE_FILE=$CORE_DIR/nios_system.sopcinfo
SOF_FILE=$CORE_DIR/output_files/custom_cpu.sof
JDI_FILE=$CORE_DIR/output_files/custom_cpu.jdi
BSP_PATH=../../bsp/mpsoc_hello_mpsoc
SRC_PATH=./src

APP=single-threaded-code        # same name as the folder
CPU=cpu
NODES=3
...
if [ ! -d ${BSP_PATH}_0 ] || [ "$REMAKE_BSP" = true ]; then
    ...
    nios2-bsp hal ${BSP_PATH}_0 $CORE_FILE \
        ...
        --set hal.make.bsp_cflags_debug -g \
  		  ...
  		  --set hal.enable_small_c_library 0 \
  		  --set hal.enable_reduced_device_drivers 0 \
...
for i in `seq 1 $NODES`; do
	...
	nios2-bsp hal ${BSP_PATH}_$i $CORE_FILE \
		  ...
		  --set hal.timestamp_timer timer_$i \
		  --set hal.enable_exit false \
		  --set hal.enable_c_plus_plus false \
		  --set hal.enable_clean_exit false \
		  --set hal.enable_sim_optimize false
...
for i in `seq 0 $NODES`; do
    ...
    xterm -title "${CPU}_$i" -e "nios2-terminal -i $i >> output$i.txt" &
done
